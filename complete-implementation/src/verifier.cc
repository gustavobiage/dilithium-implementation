...
bool tcc::verify(struct signature<K, L, N, Q, W> signature, byte * message, int message_size, struct public_key<K, L, N, Q, W> public_key) {
    ...
    byte hpt1[MESSAGE_MU_SIZE];
    tcc::sample_bytes(pt1, SEED_P_SIZE + T1_PACKED_SIZE, hpt1, MESSAGE_MU_SIZE); // TODO create sampled_bytes_size

    byte hpt1M[MESSAGE_MU_SIZE + message_size];
    memcpy(hpt1M, hpt1, MESSAGE_MU_SIZE);
    memcpy(&hpt1M[MESSAGE_MU_SIZE], message, message_size);

    byte mu[MESSAGE_MU_SIZE];
    tcc::sample_bytes(hpt1M, MESSAGE_MU_SIZE + message_size, mu, MESSAGE_MU_SIZE);
    ...
    return signature.z.norm_power_2() >=  (GAMMA1 - BETA) && c_til_are_equal && hints_equal_1_count;
}